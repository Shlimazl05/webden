...
//     onEditClick: () => void;
//     onGoToSecurity: () => void; // Thêm dòng này để fix lỗi gọi hàm
// }
...
//                 type="button"
//                 onClick={toggleMenu}
//                 className={`p-2 rounded-full transition-all ${isOpen ? 'bg-slate-100 rotate-90' : 'hover:bg-slate-100'}`}
//             >
//                 <Settings size={20} className="text-slate-400" />
//             </button>

//             {isOpen && (
//                 <div className="absolute right-0 mt-2 w-52 bg-white border border-[var(--border-subtle)] rounded-xl shadow-xl py-1 animate-in fade-in zoom-in duration-150">
//                     <button
//                         onClick={() => { onEditClick(); closeMenu(); }}
//                         className="w-full flex items-center px-4 py-2.5 text-sm font-medium hover:bg-slate-50 text-slate-700 transition-colors"
//                     >
//                         <Edit3 size={14} className="mr-2.5 text-blue-500" /> Chỉnh sửa thông tin
//                     </button>

//                     <div className="h-[1px] bg-slate-100 mx-2"></div>

//                     <button
//                         onClick={() => { onGoToSecurity(); closeMenu(); }} // Bây giờ hàm này đã tồn tại
//                         className="w-full flex items-center px-4 py-2.5 text-sm font-medium hover:bg-slate-50 text-slate-700 transition-colors"
//                     >
//                         <Lock size={14} className="mr-2.5 text-orange-500" /> Đổi mật khẩu
//                     </button>
//                 </div>
//             )}
//         </div>
