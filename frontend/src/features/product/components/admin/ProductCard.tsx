...
export const ProductCard = ({ product, onDelete, onToggleStatus, onEdit, onView }: ProductCardProps) => {
  ...
  return (
    <div
      ...
      className={`bg-white p-2.5 rounded-2xl border transition-all flex flex-col h-full group cursor-pointer relative ${!isVisibleOnClient ? 'border-slate-100 shadow-none' : 'border-slate-100 shadow-sm hover:shadow-md'
        }`}
    >
      {/* 1. Ảnh sản phẩm */}
      ...
      <div className="flex-1 flex flex-col px-1">
        <h3 className={`text-[#001529] font-bold text-[15px] leading-snug line-clamp-2 min-h-[38px] mb-2 ${!isVisibleOnClient ? 'text-slate-400' : 'text-slate-800'
          }`}>
          {product.productName}
        </h3>

        <p className={`text-[15px] font-black mb-3 ${!isVisibleOnClient ? 'text-slate-300' : 'text-red-600'
          }`}>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className={`flex-1 h-9 rounded-xl  flex items-center justify-center transition-all active:scale-90 shadow-sm ${isManualHidden
