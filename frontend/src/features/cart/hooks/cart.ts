...
export const useCart = () => {
  ...
  // --- CẬP NHẬT HÀM FETCH CART ĐỂ TỰ ĐỘNG ĐỒNG BỘ SELECTED ---
  const fetchCart = useCallback(async () => {
    if (!isLoggedIn) {
      setIsLoading(false);
      return;
    ...
    try {
      ...
      const res: any = await getCartApi();
      if (res.success) {
        // 1. Lấy danh sách ID đã chọn từ máy tính
        let storedIds = getStoredSelectedIds();
        let hasChange = false;

        // 2. Duyệt qua items từ backend
        const itemsWithSelection = res.data.items.map((item: ICartDetail) => {
          // Nếu Backend bảo item này được chọn (thường là món mới thêm)
          // mà trong máy tính chưa lưu ID này -> Thêm vào danh sách chọn
          if (item.selected && !storedIds.includes(item._id)) {
          ...
          return {
            ...item,
            // Trạng thái chọn cuối cùng phụ thuộc vào việc ID có nằm trong list stored không
            selected: storedIds.includes(item._id)
          };
        });

        // 3. Nếu có sự thay đổi (món mới được thêm vào list chọn), lưu lại localStorage
        if (hasChange) {
          localStorage.setItem(SELECTED_STORAGE_KEY, JSON.stringify(storedIds));
        }
  ...
  const removeSelected = async () => {
    ...
    try {
      ...
      setCart(prev => {
        if (!prev) return null;
        // Lọc bỏ những ID đã xóa khỏi localStorage
        const oldStored = getStoredSelectedIds();
        const newStored = oldStored.filter(id => !selectedIds.includes(id));
        localStorage.setItem(SELECTED_STORAGE_KEY, JSON.stringify(newStored));
