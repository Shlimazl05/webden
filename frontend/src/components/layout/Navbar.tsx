...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 max-w-[700px] mx-4 relative"> {/* Thêm relative ở đây để chứa dropdown kết quả */}
            ...
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-3 text-slate-300">
              ...
              >
                <Camera 
                  size={18} 
                  strokeWidth={2.5} 
                  className={`cursor-pointer transition-all duration-300 ${isVisualLoading ? 'animate-bounce text-amber-500' : 'hover:scale-110 hover:text-amber-400'}`} 
                />
              </button>
            </div>
          </div>
        ...
        {/* HIỂN THỊ KẾT QUẢ AI (Dropdown Results) */}
        {visualResults.length > 0 && (
          <div className="absolute top-[60px] left-0 w-full bg-slate-900 border border-slate-800 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden z-[60] animate-in fade-in slide-in-from-top-2">
            <div className="px-6 py-3 border-b border-slate-800 flex justify-between items-center bg-slate-800/50">
              ...
              <button onClick={() => setVisualResults([])} className="text-slate-400 hover:text-white transition-colors">
                <X size={16} />
              </button>
            </div>
            <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
              {visualResults.map((product) => (
                <Link 
                  ...
                  onClick={() => setVisualResults([])} // Click vào thì đóng dropdown
      ...
      {/* 3. Navigation Links & Account Management (Giữ nguyên phần của bạn) */}
      <div className="flex items-center gap-6 text-slate-200 font-bold text-sm flex-shrink-0 justify-end w-[420px]">
